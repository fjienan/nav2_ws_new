"""TF 广播节点：
- 静态: map → nav_map (identity，让 frame_id=map 的消息能转到 nav_map)
- 动态: nav_map → base_link (从 /odin1/relocation)
"""
...
class OdomToTfNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"TF: map→nav_map (static) + nav_map→base_link (from {pose_topic})")
